...
*                                                             file_encoding        = file_encoding
  ...
  METHOD set_alv_selected.

    FIELD-SYMBOLS: <lv_grid>  TYPE any,
                   <lv_check> TYPE any.

    DATA: ls_grid TYPE REF TO data.
    CREATE DATA ls_grid LIKE LINE OF ct_data.
    ASSIGN ls_grid->* TO <lv_grid>.
    ASSIGN COMPONENT  iv_flag  OF STRUCTURE <lv_grid> TO <lv_check>.
*
...
*
    CASE iv_mode.
      WHEN 'S'. "SINGLE
        IF lv_lines NE 1.
          MESSAGE '[단일선택가능]실행(처리)할 라인을 선택하세요.' TYPE 'I'.
          ev_error = 'X'.
          EXIT.
        ENDIF.
      WHEN 'M'. "MULTI
        IF lv_lines EQ 0 .
          MESSAGE '[복수선택가능]실행(처리)할 라인을 선택하세요.' TYPE 'I'.
          ev_error = 'X'.
          EXIT.
        ENDIF.
    ENDCASE.
*
    LOOP AT ct_data INTO <lv_grid>.
      DATA(lv_tabix) = sy-tabix.
      READ TABLE lt_row_no WITH KEY row_id = sy-tabix
                    TRANSPORTING NO FIELDS.
      IF sy-subrc EQ 0.
        <lv_check> = 'X'.
        APPEND <lv_grid> TO ET_DATA.
      ELSE.
        <lv_check> = ''.
      ENDIF.
      MODIFY ct_data FROM <lv_grid> INDEX lv_tabix.
    ENDLOOP.
