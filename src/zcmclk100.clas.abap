...
public section.
  ...
  types:
    BEGIN OF ty_bookcnt,
        se_id    TYPE hrp1000-objid,
        book_cnt TYPE zzbook_cnt,
      END OF ty_bookcnt .
  types:
    tt_bookcnt TYPE TABLE OF ty_bookcnt .
  types:
    BEGIN OF ty_credit,
  ...
  class-methods READ_HRP9550_COUNTG
    importing
      !I_SEID type PIQSEOBJID
      !I_KEYDT type DATUM
    returning
  ...
  class-methods READ_HRP9550_COUNT
    ...
    returning
      value(R_CNT) type NUMC5 .
  class-methods READ_ENROLLED_COUNTS
    importing
      !IT_SELIST type HROBJECT_T
      !I_KEYDT type DATUM
      !I_ST_REGWINDOW type PIQREGWINDOW optional
    exporting
      !ET_BOOKCNT type TT_BOOKCNT .
  class-methods READ_ENROLLED_COUNT
    importing
      !I_SEID type PIQSEOBJID
      !I_KEYDT type DATUM
      !I_ST_REGWINDOW type PIQREGWINDOW
...
CLASS ZCMCLK100 IMPLEMENTATION.
  ...
  METHOD booked_credit.
    ...
    e_cnt_t = |{ zcmclk100=>otr( 'ZDCM05/TX10000106' ) } : { e_cnt } |.
...
*     재이수여부
      ...
      <fs>-retake_txt = zcmclk100=>otr( 'ZDCM/REASSIGN_MODULE' ).
...
*경영전문대학원 별도로직
    ...
    IF lv_total > i_max_credit.
      es_msg-type = 'E'.
      es_msg-message = zcmclk100=>otr( EXPORTING alias = 'ZDCM05/TX10000059' ).
    ENDIF.
...
*신청 불가능
  ...
  METHOD check_is_bookable.
    ...
    IF ls_auth-e_dup IS NOT INITIAL.
      ...
      zcmclk100=>check_timeconf_for_event(
        EXPORTING
...
**
***분반 LOCK
**      CHECK zcmclk100=>se_lock( EXPORTING i_stid = i_stid
**                                          i_seid         = is_booking_info-seobjid
**                                          i_st_regwindow = i_st_regwindow
**                                          i_keydt        = i_keydt
**                                          i_peryr        = i_peryr
...
**        zcmclk100=>se_lock_log( i_stid  = i_stid
**                                i_smid  = is_booking_info-smobjid
**                                i_seid  = is_booking_info-seobjid
**                                i_peryr = i_peryr
**                                i_perid = i_perid ).
...
**                        CHANGING ct_msg = et_msg ) IS INITIAL.
  ...
  METHOD check_possb_org.
    ...
    IF sy-subrc <> 0.
      es_msg-type = 'E'.
      es_msg-message = zcmclk100=>otr( 'ZDCM05/TX10000067' ).
    ENDIF.
...
*선수과목의 대체과목 中 하나라도 이수했으면 통과!
    ...
    IF es_msg-type = 'E'.
      es_msg-message = zcmclk100=>otr( 'ZDCM05/TX10000055' ).
    ENDIF.
...
*정원, 신청인원
    ...
    IF ls_cnt-book_cnt >= ls_cnt-limit_kapz.
      es_msg-type = 'E'.
      es_msg-message = zcmclk100=>otr( EXPORTING alias = 'ZDCM05/TX10000058' ).
    ENDIF.
...
*{ 현재 수강 여석을 가져온다.
*    lv_booking_count = zcmclk100=>read_hrp9550_count( i_seid = i_se i_keydt = i_keydt ).
    lv_booking_count = zcmclk100=>read_enrolled_count( i_seid = i_se i_keydt = i_keydt i_st_regwindow = i_st_regwindow ).
...
*       lv_booking_count = zcmclk100=>read_hrp9550_count( i_seid = i_se i_keydt = i_keydt ).
      ...
      lv_err = zcmclk100=>enqueue_count_hrp9550( i_seid = i_se i_keydt = i_keydt ).
    ...
    IF lv_err = 'X'.
      es_msg-type = 'E'.
*      es_msg-message = |{ zcmclk100=>msg( arbgb = 'ZMCCM01' msgnr = '556' ) }  |.
      es_msg-message = zcmclk100=>otr( EXPORTING alias = 'ZDCM05/TX10000543' ).
...
*    lv_booking_count = zcmclk100=>read_hrp9550_count( i_seid = i_se i_keydt = i_keydt ).
    ...
    IF lv_booking_count >= ls_cnt-limit_kapz.
      es_msg-type = 'E'.
      es_msg-message = zcmclk100=>otr( EXPORTING alias = 'ZDCM05/TX10000058' ).
      zcmclk100=>dequeue_count_hrp9550( i_seid = i_se i_keydt = i_keydt ).
      RETURN.
    ENDIF.
...
*      COMMIT WORK.
*    ENDIF.
    zcmclk100=>update_9550( i_st_regwindow = i_st_regwindow i_se = i_se i_keydt = i_keydt i_book_cnt = lv_booking_count ).
    zcmclk100=>dequeue_count_hrp9550( i_seid = i_se i_keydt = i_keydt ).
...
*S성적은 재이수할 수 없습니다.
    IF ls_acwork_re-gradesym = 'S'.
      es_msg-type = 'E'.
      es_msg-message = zcmclk100=>otr( 'ZDCM05/TX10000070' ).
      RETURN.
    ENDIF.
...
*      es_msg-type = 'E'.
*      es_msg-message = zcmclk100=>otr( 'ZDCM05/TX10000068' ).
      CLEAR es_msg.
      RETURN.
    ENDIF.
...
*재이수 요건
    ...
    IF ls_9566 IS INITIAL.
      es_msg-type = 'E'.
      es_msg-message = zcmclk100=>otr( 'ZDCM05/TX10000093' ).
      RETURN.
    ENDIF.
...
*
  ...
  METHOD decrease_booking_count.
    ...
    DO 100 TIMES.
      CLEAR: lv_err.
      lv_err = zcmclk100=>enqueue_count_hrp9550( i_seid = i_seid i_keydt = i_keydt ).
      IF lv_err IS INITIAL.
        EXIT.
      ENDIF.
    ENDDO.
    ...
    IF lv_err = 'X'.
      zcmclk100=>dequeue_count_hrp9550( i_seid = i_seid i_keydt = i_keydt ).
      es_msg-type = 'E'.
*      es_msg-message = |{ zcmclk100=>msg( arbgb = 'ZMCCM01' msgnr = '556' ) }  |.
      es_msg-message = zcmclk100=>otr( EXPORTING alias = 'ZDCM05/TX10000543' ).
      RETURN.
...
*{ 현재 수강 여석을 가져온다.
*    lv_booking_count = zcmclk100=>read_hrp9550_count( i_seid = i_seid i_keydt = i_keydt ).
    lv_booking_count =
      zcmclk100=>read_enrolled_count( i_seid = i_seid i_keydt = i_keydt i_st_regwindow = i_st_regwindow ).
...
*      COMMIT WORK.
*    ENDIF.
    zcmclk100=>update_9550( i_st_regwindow = i_st_regwindow i_se = i_seid i_keydt = i_keydt i_book_cnt = lv_booking_count ).
    zcmclk100=>dequeue_count_hrp9550( i_seid = i_seid i_keydt = i_keydt ).
...
*        SORT lt_9550 BY objid.
        ...
        read_enrolled_counts(
          EXPORTING
            it_selist = it_selist
            i_keydt = i_keydt
            i_st_regwindow = i_st_regwindow
          IMPORTING
            et_bookcnt = DATA(lt_bookcnt)
        ).
        SORT lt_bookcnt BY se_id.
    ...
    LOOP AT it_selist INTO DATA(ls_se).
      ...
      IF i_real_506 IS NOT INITIAL.
        ...
        ENDIF.
      ELSE.
        READ TABLE lt_bookcnt INTO DATA(ls_bookcnt) WITH KEY se_id = ls_cnt-se_id BINARY SEARCH.
        IF sy-subrc = 0.
          ls_cnt-book_cnt = ls_bookcnt-book_cnt.
        ENDIF.
      ENDIF.
  ...
  METHOD read_enrolled_count.

    IF i_st_regwindow = 'G'. "교환학생
      r_cnt = zcmclk100=>read_hrp9550_countg( i_seid = i_seid i_keydt = i_keydt ).

    ELSE. "일반정규생
      r_cnt = zcmclk100=>read_hrp9550_count( i_seid = i_seid i_keydt = i_keydt ).
  ...
  METHOD read_enrolled_counts.

*   교환학생
    IF i_st_regwindow = 'G'.
      SELECT objid AS se_id, book_cntg AS book_cnt FROM hrp9550
        INTO TABLE @et_bookcnt
         FOR ALL ENTRIES IN @it_selist
       WHERE plvar = '01'
         AND otype = 'SE'
         AND istat = '1'
    ...
    ELSE. "일반정규생
      SELECT objid AS se_id, book_cnt FROM hrp9550
        INTO TABLE @et_bookcnt
       FOR ALL ENTRIES IN @it_selist
       WHERE plvar = '01'
         AND otype = 'SE'
  ...
  METHOD schedule_text.
    ...
    LOOP AT lt_week INTO DATA(lv_day).
      ...
      IF <fs> IS NOT INITIAL.
        DATA(lv_day_v) = |ZCMCLK100=>DAYS_{ lv_day }_{ i_langu }|.
...
*
  ...
  METHOD se_lock_log.
    ...
    ls_zcmt2024_lockchk-message = zcmclk100=>msg( arbgb = 'ZMCCM01' msgnr = '556' ).
  ...
  METHOD timelimit_to_text.
    ...
    IF iv_tlt IS INITIAL.
      CASE is_time-ca_timelimit.
        WHEN '0300'.
          DATA(lv_tit) = zcmclk100=>otr( 'ZDCM05/TX10000071' ).
        WHEN '0310'.
          lv_tit = zcmclk100=>otr( 'ZDCM05/TX10000072' ).
        WHEN '0320'.
          lv_tit = zcmclk100=>otr( 'ZDCM05/TX10000073' ).
        WHEN '0330'.
          lv_tit = zcmclk100=>otr( 'ZDCM05/TX10000074' ).
      ENDCASE.
    ENDIF.
